...
Delete `netlify.toml`, or remove every `protect-vip-audio` `[[edge_functions]]` block (Penny, Here Eat This, Still Standing, Jailhouse Lawyer, Chandra, `/__preview/vip-audio-diag`). Do not leave a leftover site-wide `/audio/**` rule. Echo / She Still Exists / Other Man are **not** gated (CEO HOLD).
